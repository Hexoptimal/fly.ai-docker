...
#
# The staged context is the server, the two world engine files and the connectome export (~58 MB).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$STAGE/world/src" "$STAGE/world/public" "$STAGE/mine"
...
git -C "$ROOT" diff --quiet HEAD -- mine world/src/connectome.ts world/src/rng.ts 2>/dev/null \
