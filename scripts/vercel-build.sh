#!/usr/bin/env bash
# Vercel build: the 3-D simulator, Fly Radio, Fly Roulette, Flinder, the Flybook app, fly.ai compute and the static site, assembled into .vercel-out.
# Fly Radio runs the connectome in the browser and loads the Simulation's brain files (/simulation/connectome/).
set -euo pipefail
cd "$(dirname "$0")/.."
(cd world && npm run build && npm run build:radio && npm run build:roulette && npm run build:flinder)
(cd flybook/web && npm run build)
rm -rf .vercel-out
mkdir -p .vercel-out/simulation .vercel-out/flybook .vercel-out/radio .vercel-out/roulette .vercel-out/flinder
